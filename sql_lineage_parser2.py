...
"""
================================================================================
SQL Lineage Parser V2 - MERGE INTO / INSERT INTO Native Parser
================================================================================
Extracts column-level physical data lineage from Oracle SQL DML files
(MERGE INTO and INSERT INTO SELECT). Derives target/source/expression mappings
directly from the SQL AST — no Excel mappings sheet required. Optionally accepts
a DM ATOMIC Excel for SELECT * expansion and dm_match validation.

Version: 2.0.0
Created: 2026-04-19
Based on: sql_lineage_parser.py v1.0.0

Usage:
    python sql_lineage_parser_v2.py --sql file.sql
    python sql_lineage_parser_v2.py --sql file.sql --dm-model DM_ATOMIC.xlsx
    python sql_lineage_parser_v2.py --sql-dir sql_files/ --dm-model DM_ATOMIC.xlsx --verbose
...
def load_dm_model(path: Path) -> Dict[str, Set[str]]:
    """Load DM ATOMIC sheet from Excel workbook (optional).

    Args:
        path: Path to Excel file containing a 'DM ATOMIC*' sheet.

    Returns:
        dm_dict: Dictionary mapping TABLE -> {COLUMN, ...} for O(1) lookups.
                 Returns empty dict if sheet not found or validation fails.
    """
    if not path.exists():
        logger.error(f"DM model file not found: {path}")
        sys.exit(2)

    logger.info(f"Loading DM model: {path}")
    ...
    if dm_sheet:
        ...
        logger.warning("No 'DM ATOMIC*' sheet found in file - dm_match will be 'N' for all")
...
def _expand_star_projection(scope: Scope, dm_dict: Optional[Dict[str, Set[str]]] = None) -> None:
    ...
    for alias, relation in scope.relations.items():
        if isinstance(relation, Scope):
            ...
            # FIX: If inner scope has star placeholders (*_ALIAS), propagate the
            # underlying relation's projections upward so they become visible.
            star_keys = [k for k in relation.projections if k.startswith("*_")]
            for star_key in star_keys:
                inner_alias = star_key[2:]  # strip "*_" prefix
                inner_rel = relation.relations.get(inner_alias) or relation.relations.get(inner_alias.lower())
                if inner_rel is None:
                    continue
                if isinstance(inner_rel, Scope):
                    for pname, pdef in inner_rel.projections.items():
                        if pname.upper() not in scope.projections:
                            scope.projections[pname.upper()] = ProjectionDef(
                                output_name=pname,
                                expression=pdef.expression,
                                source_refs=pdef.source_refs.copy() if pdef.source_refs else [pdef.expression],
                                origin_alias=alias
                            )
...
def resolve_to_physical(
    ...
    # STEP 1b: Oracle sequence detection (SEQ.NEXTVAL / SEQ.CURRVAL)
    alias_pre, column_pre = parse_ref(ref)
    if alias_pre and column_pre in ('NEXTVAL', 'CURRVAL'):
        return [ResolvedColumn(
            source_type=SourceType.SEQUENCE,
            table=alias_pre,
            column=column_pre,
            trace_path="->".join(trace) + f":SEQUENCE({alias_pre}.{column_pre})",
            source_alias=alias_pre,
            original_ref=ref
        )]
    ...
    if alias:
        ...
        if isinstance(relation, Scope):
            ...
            # FIX: Star-placeholder fallback — if the scope has *_X placeholders,
            # resolve the column against the underlying relation (physical table or scope).
            for pkey in relation.projections:
                if pkey.startswith("*_"):
                    star_alias = pkey[2:]
                    star_rel = relation.relations.get(star_alias) or relation.relations.get(star_alias.lower())
                    if isinstance(star_rel, str):
                        # Physical table — resolve directly
                        dm_match_val = "Y" if column_exists_in_dm(star_rel, column, dm_dict) else "N"
                        return [ResolvedColumn(
                            source_type=SourceType.PHYSICAL,
                            table=star_rel,
                            column=column,
                            dm_match=dm_match_val,
                            trace_path="->".join(trace) + f":{star_rel}.{column}",
                            source_alias=alias,
                            original_ref=ref
                        )]
                    elif isinstance(star_rel, Scope):
                        # Nested scope — try resolving in it
                        inner_results = resolve_to_physical(
                            column, star_rel, dm_dict,
                            visited.copy(), trace + [star_rel.name], max_depth
                        )
                        if inner_results and inner_results[0].source_type != SourceType.UNRESOLVED:
                            return inner_results

            # FIX: Physical-table fallback — when resolving an expression like
            # a.N_CCF inside a scope where 'a' is a physical table, but the lookup
            # landed on a parent scope where 'a' is a subquery wrapping that table.
            # Check the scope's own relations for physical tables that have the column.
            for rel_alias, rel in relation.relations.items():
                if isinstance(rel, str):
                    # Physical table — resolve directly
                    dm_match_val = "Y" if column_exists_in_dm(rel, column, dm_dict) else "N"
                    return [ResolvedColumn(
                        source_type=SourceType.PHYSICAL,
                        table=rel,
                        column=column,
                        dm_match=dm_match_val,
                        trace_path="->".join(trace) + f":{rel}.{column}",
                        source_alias=alias,
                        original_ref=ref
                    )]
...
# =============================================================================
# SECTION 9: SQL DML PARSING & MAIN PIPELINE (MERGE INTO + INSERT INTO)
# =============================================================================


def parse_merge_statement(ast) -> dict:
    """Parse a MERGE INTO AST node and extract its structural components.

    Returns:
        Dictionary with keys: target_table, target_alias, using_node,
        using_alias, on_condition, when_clauses.
    """
    if not isinstance(ast, exp.Merge):
        raise ValueError(f"Expected MERGE statement, got {type(ast).__name__}")

    # Target table
    target = ast.this
    if not target:
        raise ValueError("MERGE statement missing target table")
    target_table = target.name.upper() if hasattr(target, 'name') else str(target).upper()
    target_alias = (target.alias or target.name).upper() if hasattr(target, 'alias') else target_table

    # USING clause
    using_node = ast.args.get('using')
    if not using_node:
        raise ValueError("MERGE statement missing USING clause")
    if hasattr(using_node, 'alias') and using_node.alias:
        using_alias = using_node.alias.upper()
    else:
        using_alias = 'SRC'

    # ON condition
    on_expr = ast.args.get('on')
    if not on_expr:
        raise ValueError("MERGE statement missing ON condition")
    on_condition = on_expr.this if isinstance(on_expr, exp.Paren) else on_expr

    # WHEN clauses — stored in ast.args['whens'].expressions
    when_clauses = []
    whens_node = ast.args.get('whens')
    if whens_node and hasattr(whens_node, 'expressions'):
        when_clauses = list(whens_node.expressions)

    return {
        'target_table': target_table,
        'target_alias': target_alias,
        'using_node': using_node,
...
def extract_merge_assignments(
    when_clauses: list,
    target_table: str,
    target_alias: str
) -> list:
    """Extract (target_column, source_expression, clause_type) from WHEN clauses.

    Returns:
        List of tuples: (target_column, source_expression_sql, clause_type)
        where clause_type is 'UPDATE_SET' or 'INSERT_VALUES'.
    """
    assignments = []

    for when_node in when_clauses:
        then_expr = when_node.args.get('then')
        if not then_expr:
            continue

        # WHEN MATCHED THEN UPDATE SET ...
        if isinstance(then_expr, exp.Update):
            set_exprs = then_expr.expressions or []
            for eq_node in set_exprs:
                if isinstance(eq_node, exp.EQ):
                    left_sql = eq_node.this.sql(dialect="oracle")
                    left_col = left_sql.upper().strip()
                    for prefix in [f"{target_alias}.", f"{target_table}."]:
                        if left_col.startswith(prefix):
                            left_col = left_col[len(prefix):]
                            break
                    right_sql = eq_node.expression.sql(dialect="oracle")
                    assignments.append((left_col, right_sql, 'UPDATE_SET'))

        # WHEN NOT MATCHED THEN INSERT (...) VALUES (...)
        elif isinstance(then_expr, exp.Insert):
            col_list = then_expr.this
            columns = []
            if col_list and hasattr(col_list, 'expressions'):
                for col_node in col_list.expressions:
                    col_name = col_node.name.upper() if hasattr(col_node, 'name') else col_node.sql(dialect="oracle").upper()
                    columns.append(col_name)

            values_node = then_expr.expression
            values = []
            if values_node and hasattr(values_node, 'expressions'):
                for val_node in values_node.expressions:
                    val_sql = val_node.sql(dialect="oracle")
                    values.append(val_sql)

            for col, val in zip(columns, values):
                assignments.append((col, val, 'INSERT_VALUES'))

        # WHEN MATCHED THEN DELETE -- skip
        else:
            logger.debug(f"Skipping WHEN clause with action: {type(then_expr).__name__}")

    return assignments


def build_using_scope(
    using_node,
    using_alias: str,
    dm_dict: dict
) -> Scope:
    """Build a scope tree from the MERGE USING clause.

    Handles:
    1. USING (SELECT ...) alias  -- subquery: build scope from inner SELECT
    2. USING table alias         -- plain table: create scope with table as relation
    """
    if isinstance(using_node, exp.Subquery):
        inner = using_node.this
        if inner:
            scope = build_scope_tree(inner, name="USING", dm_dict=dm_dict)
            # Register the USING alias so "src.col" resolves through projections.
            # Create a child scope that mirrors the USING scope's projections.
            child = Scope(name=using_alias, parent=scope)
            child.relations = dict(scope.relations)
            child.projections = dict(scope.projections)
            scope.relations[using_alias] = child
            return scope
        return Scope(name="USING")

    if isinstance(using_node, exp.Table):
        scope = Scope(name="USING")
        table_name = using_node.name.upper()
        alias = (using_node.alias or using_node.name).upper()
        scope.relations[alias] = table_name
        if dm_dict and table_name in dm_dict:
            for col in dm_dict[table_name]:
                scope.projections[col] = ProjectionDef(
                    output_name=col,
                    expression=f"{alias}.{col}",
                    source_refs=[f"{alias}.{col}"],
                    origin_alias=alias
                )
        return scope

    # Fallback: try to find a SELECT inside
    select_expr = using_node.find(exp.Select)
    if select_expr:
        return build_scope_tree(select_expr, name="USING", dm_dict=dm_dict)

    logger.warning(f"Could not parse USING clause type: {type(using_node).__name__}")
    return Scope(name="USING")


def build_merge_on_join(
    on_condition,
    target_table: str,
    target_alias: str,
    using_alias: str,
    ...
    """Convert MERGE ON condition into JoinKey objects."""
    joins = []
    join_seq = 0
    ...
    for eq_node in eq_nodes:
        ...
        def parse_ref(ref_sql):
            parts = ref_sql.strip().split('.')
            if len(parts) >= 2:
                return parts[-2].upper(), parts[-1].upper()
            return '', parts[-1].upper()

        left_tbl, left_col = parse_ref(left_sql)
        right_tbl, right_col = parse_ref(right_sql)

        joins.append(JoinKey(
            sql_file=sql_file,
            join_seq=join_seq,
            join_type='MERGE',
            left_table=left_tbl or target_alias,
            left_field=left_col,
            right_table=right_tbl or using_alias,
            right_field=right_col,
            join_condition=f"{left_sql} = {right_sql}",
            join_filters='',
            context_path='MERGE_ON'
        ))
...
def parse_insert_statement(ast) -> dict:
    """Parse an INSERT INTO AST node and extract its structural components.

    Returns:
        Dictionary with keys: target_table, target_columns, select_node.
    """
    if not isinstance(ast, exp.Insert):
        raise ValueError(f"Expected INSERT statement, got {type(ast).__name__}")

    # Target table — ast.this is the Schema/Table node
    target_node = ast.this
    if not target_node:
        raise ValueError("INSERT statement missing target table")

    # sqlglot wraps INSERT INTO table(cols) as Schema(this=Table, expressions=[Column...])
    if isinstance(target_node, exp.Schema):
        table_node = target_node.this
        target_table = table_node.name.upper() if hasattr(table_node, 'name') else str(table_node).upper()
        # Column list from Schema.expressions
        target_columns = []
        for col_node in (target_node.expressions or []):
            col_name = col_node.name.upper() if hasattr(col_node, 'name') else col_node.sql(dialect="oracle").upper()
            target_columns.append(col_name)
    elif isinstance(target_node, exp.Table):
        target_table = target_node.name.upper()
        target_columns = []  # No explicit column list
    else:
        target_table = str(target_node).upper()
        target_columns = []

    # SELECT — ast.expression is the source query
    select_node = ast.expression
    if not select_node:
        raise ValueError("INSERT statement missing SELECT clause")
...
def build_insert_scope(select_node, dm_dict: dict) -> Scope:
    """Build a scope tree from the INSERT's SELECT query."""
    return build_scope_tree(select_node, name="INSERT_SELECT", dm_dict=dm_dict)


def extract_insert_assignments(target_columns: list, scope: Scope) -> list:
    """Pair INSERT column list with SELECT projections by position.

    Returns:
        List of tuples: (target_column, source_expression_sql, 'INSERT_VALUES')
    """
    assignments = []

    # Get projections in insertion order
    proj_list = list(scope.projections.items())

    if target_columns:
        # Explicit column list — pair by position
        for i, target_col in enumerate(target_columns):
            if i < len(proj_list):
                proj_name, proj_def = proj_list[i]
                assignments.append((target_col, proj_def.expression, 'INSERT_VALUES'))
            else:
                logger.warning(f"INSERT column {target_col} at position {i} has no matching SELECT expression")
    else:
        # No explicit column list — use SELECT projection names as target columns
        for proj_name, proj_def in proj_list:
            assignments.append((proj_name, proj_def.expression, 'INSERT_VALUES'))
...
def process_sql_file(
    ...
    if isinstance(ast, exp.Merge):
        ...
        target_alias = merge_info['target_alias']
        using_node = merge_info['using_node']
        using_alias = merge_info['using_alias']
        on_condition = merge_info['on_condition']
        when_clauses = merge_info['when_clauses']

        scope = build_using_scope(using_node, using_alias, dm_dict)
        assignments = extract_merge_assignments(when_clauses, target_table, target_alias)

        # Deduplicate: prefer UPDATE_SET over INSERT_VALUES for same target column
        seen_targets = set()
        unique_assignments = []
        for target_col, source_expr, clause_type in assignments:
            if target_col not in seen_targets:
                seen_targets.add(target_col)
                unique_assignments.append((target_col, source_expr, clause_type))

        logger.info(f"MERGE {target_table}: {len(unique_assignments)} column assignments from {len(assignments)} total")
        stmt_type = "MERGE"
        ...
        target_alias = target_table
        select_node = insert_info['select_node']

        scope = build_insert_scope(select_node, dm_dict)
        unique_assignments = extract_insert_assignments(insert_info['target_columns'], scope)

        logger.info(f"INSERT {target_table}: {len(unique_assignments)} column assignments")
        on_condition = None
        using_alias = None
        stmt_type = "INSERT"
    ...
    # -------------------------------------------------------------------------
    # Common resolution + output (shared by both MERGE and INSERT)
    # -------------------------------------------------------------------------
    result = LineageResult(object_name=object_name, sql_content=sql_content, warnings=warnings)

    # Resolve each assignment
    for target_col, source_expr, clause_type in unique_assignments:
        resolved_columns = resolve_with_fallback(source_expr, scope, dm_dict, max_depth)
        ...
        for resolved in resolved_columns:
            edge = LineageEdge(
                ...
                dm_match=resolved.dm_match,
                trace_path=resolved.trace_path,
                notes=f"{clause_type}: {resolved.reason}" if resolved.reason else clause_type,
                source_alias=resolved.source_alias,
                original_ref=resolved.original_ref,
                full_expression=resolved.full_expression or source_expr
            )
            result.edges.append(edge)

    # Collect joins from scope tree
    joins_with_scopes = collect_joins_with_scopes(scope)
    for join, _ in joins_with_scopes:
        join.sql_file = sql_file_name

    # Add MERGE ON condition as a join relationship (MERGE only)
    if stmt_type == "MERGE" and on_condition is not None:
        merge_on_joins = build_merge_on_join(
            on_condition, target_table, target_alias, using_alias,
            object_name=object_name, sql_file=sql_file_name
        )
        for mj in merge_on_joins:
            joins_with_scopes.append((mj, scope))

    # Extract just the joins for result
    result.joins = [join for join, _ in joins_with_scopes]

    # Explode join keys to physical sources
    ...
    # Convert JoinKeyExploded to LineageEdge and append
    for jke in result.joins_exploded:
        # Skip MERGE ON target-side refs — they resolve as UNRESOLVED because
        # the target alias (T) isn't a source. These are routing logic, not lineage.
        if jke.join_type == 'MERGE' and jke.source_type == SourceType.UNRESOLVED.value:
            continue
    ...
    result.stats = {
        ...
        'sequence_count': sum(1 for e in result.edges if e.source_type == SourceType.SEQUENCE.value),
        ...
        try:
            if separate:
                ...
                result = process_sql_file(
                    sql_file, dm_dict, output_dir, max_depth,
                ...
                if result:
                    ...
                    print(" SKIPPED")
                    skipped += 1
                    manifest["files"][obj_name] = {"status": "skipped", "reason": "skipped"}
            else:
                # Combined mode: collect results, write at end
                result = process_sql_file(
                    sql_file, dm_dict, None, max_depth,
                    sql_file_name=sql_file.stem
                )
...
def dry_run(dm_model: Optional[Path], sql_source: Path) -> None:
    """Validate inputs without processing."""
    print("=== DRY RUN MODE ===\n")

    # Load DM model (optional)
    dm_dict: Dict[str, Set[str]] = {}
    if dm_model:
        dm_dict = load_dm_model(dm_model)
        print(f"DM model loaded: {len(dm_dict)} tables")
    else:
        print("No DM model provided (dm_match will be 'N' for all)")
    ...
    # Check each SQL file contains MERGE INTO
    would_process = []
    would_skip = []

    for sql_file in sql_files:
        try:
            _, sql_content = load_sql_file(sql_file)
            valid, sql_clean, _ = validate_sql_content(sql_content, sql_file)
            if valid:
                sql_normalized = normalize_sql(sql_clean)
                ast = sqlglot.parse_one(sql_normalized, dialect="oracle")
                if isinstance(ast, exp.Merge):
                    ...
                    would_skip.append((sql_file.name, f"Not MERGE (got {type(ast).__name__})"))
            else:
                would_skip.append((sql_file.name, "Invalid SQL"))
        except Exception as e:
            would_skip.append((sql_file.name, str(e)))

    print(f"\nWould process (MERGE INTO): {len(would_process)}")
    print(f"Would skip: {len(would_skip)}")

    if would_skip:
        for name, reason in would_skip[:5]:
            print(f"  Skip: {name} - {reason}")
        if len(would_skip) > 5:
            print(f"  ... and {len(would_skip) - 5} more")

    print(f"\nEstimated output: {len(would_process)} file(s)")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description='SQL Lineage Parser V2 - Extract column-level lineage from Oracle MERGE INTO SQL',
...
Examples:
  ...
  %(prog)s --sql-dir sql_files/ --dry-run
    ...
    # Optional DM model
    parser.add_argument('--dm-model', type=Path, default=None,
                        help='Path to Excel with DM ATOMIC sheet (optional, for SELECT * expansion and dm_match)')
    ...
    # Dry run mode
    if args.dry_run:
        dry_run(args.dm_model, sql_source)
        return 0

    # Load DM model (optional)
    dm_dict: Dict[str, Set[str]] = {}
    if args.dm_model:
        dm_dict = load_dm_model(args.dm_model)

    # Process
    if args.sql:
        # Single file mode
        result = process_sql_file(
            args.sql, dm_dict, args.output, args.max_depth
        )
        if result:
            print(f"\nOutput: {result}")
            return 0
